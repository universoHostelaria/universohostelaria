...
export default function CartDrawer() {
  ...
  const buildWhatsAppMessage = () => {
    const lines = items.map(i =>
      `• ${i.product.name} (${supplierName(i.product.supplier_id)}) × ${i.quantity} ud — ${
        i.product.price
          ? (i.product.price * i.quantity).toFixed(2).replace('.', ',') + ' €'
          : 'precio a consultar'
      }`
    )
    const totalLine = total > 0 ? `\n*Total estimado: ${total.toFixed(2).replace('.', ',')} € + IVA*` : ''
    const msg = `Hola, me interesa hacer un pedido:\n\n${lines.join('\n')}${totalLine}\n\n¿Podéis confirmar disponibilidad y plazos?`
  ...
  const buildEmailBody = () => {
    ...
    const totalLine = total > 0 ? `\nTotal estimado: ${total.toFixed(2).replace('.', ',')} € + IVA` : ''
    const body = `Hola,\n\nMe interesa hacer el siguiente pedido:\n\n${lines.join('\n')}${totalLine}\n\nPor favor confirmad disponibilidad y plazos.\n\nGracias.`
  ...
  const handleEmail = () => {
    window.location.href = `mailto:hola@universohosteleria.es?subject=Solicitud de pedido&body=${buildEmailBody()}`
  }
                ...
                <div className={styles.itemInfo}>
                  ...
                  {item.product.price ? (
                    <div className={styles.itemPrice}>
                      {(item.product.price * item.quantity).toFixed(2).replace('.', ',')} € <span>+ IVA</span>
                    </div>
                  ) : (
          ...
          <div className={styles.footer}>
            {total > 0 && (
              <div className={styles.totalRow}>
                ...
                <span className={styles.totalVal}>{total.toFixed(2).replace('.', ',')} € <small>+ IVA</small></span>
              </div>
            )}
            <p className={styles.footerNote}>
              Nuestro especialista confirmará disponibilidad, plazos y precio final.
            </p>
            <button className={styles.btnWa} onClick={handleWhatsApp}>
            ...
            <button className={styles.btnEmail} onClick={handleEmail}>
